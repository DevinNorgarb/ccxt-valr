...
class valr extends valr$1 {
    ...
    parseBalance(balances) {
        ...
        for (let i = 0; i < balances.length; i++) {
            ...
            const debt = Precise["default"].stringAdd(this.safeString(balance, 'lendReserved'), this.safeString(balance, 'borrowReserved'));
            result[code] = {
    ...
    async fetchTradingFees(params = {}) {
        ...
        for (let i = 0; i < response.length; i++) {
            ...
            if (('makerPercentage' in tradeFee) && ('takerPercentage' in tradeFee)) {
                ...
                makerStr = (makerStr) ? Precise["default"].stringDiv(makerStr, '100') : undefined;
                takerStr = (takerStr) ? Precise["default"].stringDiv(takerStr, '100') : undefined;
                result[symbol] = {
    ...
    async fetchDeposits(code = undefined, since = undefined, limit = undefined, params = {}) {
        ...
        if (this.isFiat(code)) {
            throw new errors.NotSupported(this.id + ' fetchDeposits() is not supported yet for fiat currencies');
        }
        const currency = this.safeCurrency(code);
        const currencyId = {
            'currency': currency['id'],
    ...
    async fetchWithdrawals(code = undefined, since = undefined, limit = undefined, params = {}) {
        ...
        if (this.isFiat(code)) {
            throw new errors.NotSupported(this.id + ' fetchWithdrawals() is not supported yet for fiat currencies');
        }
        const currency = {
